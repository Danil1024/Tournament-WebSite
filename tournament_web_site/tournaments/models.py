...
class Tournament(models.Model):
    ...
    def clean(self):
        ...
        if self.start_date < self.registration_start_date:
            raise ValidationError({
                'start_date': 'Дата начала турнира не может быть раньше даты начала регистрации.'
            })

        if self.start_date - self.registration_start_date < timedelta(days=1):
            raise ValidationError({
                'start_date': 'Между началом регистрации и началом турнира должен быть минимум 1 день.'
            })
